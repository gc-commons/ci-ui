    ...
    .directive('ciUiFavicon', [
      ...
      function(uiDefaults) {
        return {
          ...
          link: function(scope, elem) {
            scope.$on(scope.event || uiDefaults.onEvent, function(e, brandSrv) {
              elem.attr('href', decodeURIComponent(brandSrv.getFavicon()));
            });
          }
        }
   ...
   */
    .directive('ciUiLogo', [
      function() {
        return {
          restrict: 'A',
          template: '<img ng-if="url" ng-src="{{url}}" alt="{{alt}}" class="_ci-ui-logo" />',
          scope: {
            url: '@',
            alt: '@'
          }
        ...
        <div class="btn-group" dropdown> \
          ...
          <ul class="dropdown-menu" role="menu" aria-labelledby="ci-dropdown-label"> \
            <li role="menuitem" ng-repeat="link in dd.links"> \
              <a ng-click="{{ link.url }}"><i id="{{ link.id }}"></i> {{ link.name }}</a> \
            </li> \
          </ul> \
        </div>';
